...
pdfDownloadButton <- function(outputId){
  ...
  tags$a(id = outputId, class = "btn btn-xs shiny-download-link", href = "", 
         target = "_blank", download = NA, icon("file-pdf"), label)
}
...
# removeWord removes special formatting from a character string--------------
removeWord <- function(complexstring) {
  # are there special formatting encoding characters? strip them
  return(gsub("<!--.*?<!\\[endif\\]-->", "", complexstring))
}
...
###############################
# find string common to all file names in dataset
# useful if files have prefix with date, etc, before tablename
...
# If common prefix detected. remove from all string vector elements
stripPrefixes <- function(strings){
  prefix <- find_common_start(strings)
  if (is.null(prefix)) return(strings)
  result <- str_split(strings, prefix)
  return(sapply(result, "[[",2))
}
